...
#这里写自己服务的名字(指定为部署jar包名,不包含 .jar )
SERVICE_NAME="order-service"
# 进程号存放文件,和xxx-xxx.jar同目录
PID_FILE="../$SERVICE_NAME.pid"
BASE_DIR=$(dirname $0)
# 由于采用single jar的模式,classpath无需指定了,由程序内部路径加载
...
JAVA_OPT_5="-Djava.ext.dirs=${JAVA_HOME}/jre/lib/ext -Dspring.profiles.active=test"
...
function startServiceProcess {
   touch $PID_FILE
   rm -rf nohup.log
   nohup $JAVA $JAVA_OPTS -jar $SERVICE_NAME.jar $KEYWORD >> nohup.log 2>&1 & echo $! > $PID_FILE
   sleep 0.1
   pid="$(<$PID_FILE)"
