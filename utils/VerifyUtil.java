...
public class VerifyUtil {
	...
	public static boolean verifyPassword(String password){
		return match(password,"^(?=.*[0-9])(?=.*[a-zA-Z])([a-zA-Z0-9]{6,16})$" );
	}
	...
	//验证身份证  大于18岁
	public static boolean checkCardID(String id){
		if(!verifyCardID(id)) return false;
		Integer age = null;
		Integer nowAge=Integer.parseInt(DateFormat.formatDate(DateFormat.YEARMONTHDAY, new Date()));
		if(id.length() ==  18){
			 age=Integer.parseInt(id.substring(6, 14));
		}else{
			 age=Integer.parseInt("19"+id.substring(6, 12));
		}
		if (nowAge-age<180000){
			return false;
        }
		return true;
	}
	 ...
	 * @return
	 */
	public static boolean verifyZipCode(String id) {
		return match(id,"\\d{6}(?!\\d)");
	}
